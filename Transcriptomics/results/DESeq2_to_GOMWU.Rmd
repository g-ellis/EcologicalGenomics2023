...
Setting up counts table and conditions table for DESeq2
```{r}
...
DESEQ2 MODEL NUMBER 2 - subset to focus on effect of treatment for each generation
```{r}
...
# Filter 
dds <- dds[rowSums(counts(dds) >= 30) >= 28,] # min of 30 reads in 75% of samples
nrow(dds) 

# Subset the DESeqDataSet to the specific level of the "generation" factor, change this for the DESeq2 model and then look for comparison between treatments
...
Save all the results as csv to go into GOMWU - OW vs. AM
```{r}
...
Save all the results as csv to go into GOMWU - OWA vs. AM
```{r}
...
Save all the results as csv to go into GOMWU - OA vs. AM
```{r}
